...
class Product:
    ...
    def add_product(self):
        if self.validation():
            ...
            self.message['text'] = 'El producto {} fue añadido satisfactoriamente'.format(self.name.get())
            self.name.delete(0, END)
            self.price.delete(0, END)
        else:
            self.message['text'] = 'Se requiere que ingrese un nombre y un precio'
        self.get_products()
    ...
    def delete_product(self):
        ...
        try:
           self.tree.item(self.tree.selection())['text'][0]
            ...
            self.message['text'] = 'Se debe seleccionar un registro'
            return
        self.message['text'] = ''
        name = self.tree.item(self.tree.selection())['text']
        query = 'DELETE FROM product WHERE name = ?'
        self.run_query(query, (name, ))
        self.message['text'] = 'El registro {} se ha eliminado satisfactoriamente'.format(name)
        self.get_products()
    ...
    def edit_records(self, new_name, name, new_price, old_price):
        ...
        self.message['text'] = 'El registro {} se ha actualizado satisfactoriamente'.format(name)
        self.get_products()
